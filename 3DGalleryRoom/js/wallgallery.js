...
var Gallery = (function () {
  ...
  function Room($items) {
    this.$el = $(
      '<div class="gr-room"><div class="gr-wall-main"><div class="gr-floor"></div></div></div>'
    ).insertAfter($itemsContainer);
    ...
    this.transitionSettings =
      ...
      "ms " +
      Gallery.settings.easing;
    // element representing the wall
    this.$mainWall = this.$el
      .find("div.gr-wall-main")
      .css("transition", this.transitionSettings);
  ...
  Room.prototype = {
    create: function () {
      ...
      this.layout =
        typeof Gallery.settings.layout != "undefined" &&
        Gallery.settings.layout instanceof Array &&
      ...
      if (this.totalItems > 1) {
        this.$navPrev = $('<span class="gr-prev">prev</span>').on(
          "click",
          $.proxy(this.navigate, this, "prev")
        );
        this.$navNext = $('<span class="gr-next">next</span>').on(
          "click",
      ...
      this.$caption = $(
        '<div class="gr-caption"><span class="gr-caption-close">x</span></div>'
      ).appendTo($gallery);
      this.$caption
        .find("span.gr-caption-close")
        .on("click", $.proxy(this.hideDescription, this));
    ...
    getLayoutSettings: function () {
      ...
      return support.transforms3d
        ? [perwall, perwall, perwall, lastwall]
        : [this.totalItems];
    },
    ...
    renderWall: function ($wallElem) {
      ...
      var wallWidth =
        wallMarginLeft === 0
          ? winsize.width
          : Math.ceil(wall.itemsCount * winsize.width);
      $wallElem
        .css("width", wallWidth)
        .find("div.gr-floor")
        .css("width", wallWidth);
    ...
    changeWall: function (dir) {
      ...
      this.walls[this.lastWall].$items.css(
        "left",
        "+=" + this.$mainWall.data("translationVal")
      );
      ...
      // the incoming wall..
      this.$auxWall = $(
        '<div class="gr-wall-other"><div class="gr-floor"></div></div>'
      ).insertAfter(this.$mainWall);
      ...
      var auxWallWidth = this.$auxWall.width(),
        auxWallInitialTranslationVal =
          dir === "next"
            ? winsize.width
            : auxWallWidth * -1 + (auxWallWidth - winsize.width) * 1,
        auxWallInitialAngle = dir === "next" ? -90 : 90,
        auxWallTransform = support.transforms3d
          ? "translate3d(" +
            auxWallInitialTranslationVal +
            "px,0px,0px) rotate3d(0,1,0," +
            auxWallInitialAngle +
            "deg)"
      ...
      if (dir === "prev") {
        this.walls[this.currentWall].$items.css(
          "left",
          "-=" + (auxWallWidth - winsize.width)
        );
      ...
      setTimeout(
        $.proxy(function () {
          var translationVal = this.$mainWall.data("translationVal") || 0,
            mainWallFinalTranslationVal =
              dir === "next"
                ? -winsize.width
                : (translationVal - winsize.width) * -1,
            ...
            .on(
              ...
              $.proxy(function () {
                ...
                this.$auxWall
                  ...
                  .css("width", this.$auxWall.data("width"));
    ...
    switchWalls: function () {
      ...
      this.$mainWall = this.$auxWall
        .addClass("gr-wall-main")
        .removeClass("gr-wall-other");
    },
      ...
      var jumpnow = $.proxy(function () {
        ...
        if (support.transitions && support.transforms3d) {
          this.$mainWall
            ...
            .on(transEndEventName, $.proxy(afterAnim, this));
        } else {
          this.$mainWall
            .stop()
            .animate(
    ...
    showDescription: function ($descriptionEl, idx) {
      var showdescfn = $.proxy(function () {
        this.$caption
          ...
          .end()
          .append(
            '<div class="gr-caption-inner">' + $descriptionEl.html() + "</div>"
          )
