...
(function (window) {
  ...
  function init() {
    ...
    move({ transition: initTransition, transform: initTransform }).then(
      function () {
        initTilt();
      }
    );
  ...
  function toggleSlide(dir, delay) {
    ...
    anime.remove([name, title, date, author]);
    var animeOpts = {
  ...
  function showInfo() {
    ...
    var aboutRoom = DOM.rooms[currentRoom].querySelector(".room_about")
      .innerHTML;
  ...
  // Zoom/Modal nas imagens da galeria
  var bodyGeral = $("body"),
  ...
  // ImageModal | Open
  $(".room__side img").on("click", function () {
    console.log("click");
    bodyGeral.addClass("modal-open");
    bodyGeral.removeClass("modal-close");

    modalGallery.fadeIn();
    var urlImg = $(this).next(".img-original").attr("src");
    var borda = $(this).data("borda") ? $(this).data("borda") : "transparent";
  ...
  jQuery(document).on("click", function (e) {
    if (
      $(e.target).closest(".room__side img", "#modalGallery .modal-content")
        .length === 0
    ) {
      modalGallery.delay(100).fadeOut();
      bodyGeral.addClass("modal-close");
